...
void UserManager::addUser(const Parser &p) {
    if (indexPool.empty()) {
        ...
        indexPool.insert(hashUsername(newUser.username), newUser);
...
void UserManager::login(const Parser &p) {
    if (!isLogin(p["-u"])) {
        ...
        if (temp.first.password == p["-p"]) {
            loginPool[p["-u"]] = temp.first.privilege;
            return outputSuccess();
        }
    }
    outputFailure();
...
void UserManager::queryProfile(const Parser &p) {
    if (isLogin(p["-c"])) {
        if (p["-c"] == p["-u"])return printUser(indexPool.find(hashUsername(p["-u"])).first);
        if (isLogin(p["-u"]) && loginPool[p["-c"]] > loginPool[p["-u"]])return printUser(indexPool.find(hashUsername(p["-u"])).first);
        std::pair<user_t, bool> temp {indexPool.find(hashUsername(p["-u"]))};
        if (!temp.second)return outputFailure();
        if (loginPool[p["-c"]] > temp.first.privilege)return printUser(temp.first);
    }
    outputFailure();
}
...
void UserManager::modifyProfile(const Parser &p) {
    if (isLogin(p["-c"]) && (!p.haveThisArgument("-g") || p("-g") < loginPool[p["-c"]])) {
        if (p["-c"] == p["-u"]) {
            user_t mUser {indexPool.find(hashUsername(p["-u"])).first};
